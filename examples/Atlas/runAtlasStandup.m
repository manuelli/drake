...
function runAtlasStandup(use_mex)
...
% this addpathTemporary isn't working, it is taking drake as a the 
path_handle_2 = addpathTemporary([getenv('DRC_BASE'),'/software/models/atlas_v5/model_minimal_contact.urdf']);
addpath([getenv('DRC_BASE'),'/software/control/matlab/planners/prone']);

% will just use this as a utility method
kpt = KinematicPoseTrajectory(r,{},1);
r = kpt.addSpecifiedCollisionGeometryToRobot({'l_hand'});
...
% I don't need this part . . ., just want to track joint accelerations
...
qp = QPControllerContacts(r,{},ctrl_data,options);
clear options;
...
% This isn't needed now that we aren't using the FootContactBlock
% ins(2).system = 1;
% ins(2).input = 3;
...
% NO LONGER USING FootContactBlock
...
function addContactGeometries(r)
