classdef QPInput3D
  ...
  methods
    function obj = QPInput3D()
      obj.timestamp = 0;
      obj.zmp_data = struct('A',  [zeros(3),eye(3); zeros(3,6)],... % COM state map % 16 d
                        'B', [zeros(3); eye(3)],... % COM input map % 4 d
                        'C', [eye(3),zeros(3)],... % ZMP state-output map % 4 d
                        'D', -0.89/9.81*eye(3),... % ZMP input-output map % 2 d
                        'x0', zeros(6,1),... % nominal state % 4 d
                        'y0', zeros(3,1),... % nominal output % 2 d
                        'u0', zeros(3,1),... % nominal input % 2 d
                        'R', zeros(3),... % input LQR cost % 4 d
                        'Qy', eye(3),... % output LQR cost % 4 d
                        'S', zeros(6),... % cost-to-go terms: x'Sx + x's1 + s2 % 16 d
                        's1', zeros(6,1),... % 4 d
                        's1dot', zeros(6,1),... % 4 d
                        's2', 0,... % 1 d
                        's2dot', 0); % 1 d
