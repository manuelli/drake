classdef QPControllerContacts < MIMODrakeSystem
  ...
  % costs/constraints.
  methods
    function obj = QPControllerContacts(r,body_accel_input_frames,controller_data,options)
    ...
    function varargout=mimoOutput(obj,t,~,varargin)
      ...
      % to see which contact indices are really active
      ...
      if (obj.use_mex==0 || obj.use_mex==2)
        ...
        % Hard coded, will need to be changed so that they work with the
        % quaterion formulation
        dim = 3; % 3D
        ...
        if ~isempty(active_supports)
          ...
          for j=1:length(active_supports)
            ...
            [~,Jp_j] = forwardKin(r,kinsol,active_supports(j),pts,0);
            Jpdot_j = forwardJacDot(r,kinsol,active_supports(j),pts,0);
            Jp = [Jp;Jp_j];
            Jpdot = [Jpdot;Jpdot_j];
          end
