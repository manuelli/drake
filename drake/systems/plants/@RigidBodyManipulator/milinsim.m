function traj = milinsim(obj,qstar,x0,h,N,mu)
...
[H,C,~,~,dC] = manipulatorDynamics(obj,qstar,zeros(nv,1));
...
for i=1:N
  ...
  model.rhs(dynamic_inds) = -C - Aq*(q-qstar) - Av*v ; 

  model.A(nonpen_lb_inds,vn_inds) = h*n;
  model.rhs(nonpen_lb_inds) = -phi - n*(q-qstar);
  model.A(nonpen_ub_inds,vn_inds) = h*n;
  model.rhs(nonpen_ub_inds) = bigM - phi - n*(q-qstar);
